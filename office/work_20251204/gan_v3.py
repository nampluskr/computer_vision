...
def sample_latent_with_labels(num_samples, latent_dim, num_classes, labels=None):
    noises = torch.randn(num_samples, latent_dim)
    if labels is None:
    ...
    return torch.cat([noises, labels_onehot], dim=1)


def sample_latent_with_codes(num_samples, latent_dim, num_continuous, num_discrete):
    noises = torch.randn(num_samples, latent_dim)
    if num_continuous > 0:
        codes_continuous = torch.randn(num_samples, num_continuous)
    else:
        codes_continuous = torch.empty(num_samples, 0)
    if num_discrete > 0:
        codes_discrete = torch.randint(0, num_discrete, (num_samples,))
        codes_discrete_onehot = F.one_hot(codes_discrete, num_discrete).float()
    else:
        codes_discrete_onehot = torch.empty(num_samples, 0)
    return torch.cat([noises, codes_continuous, codes_discrete_onehot], dim=1)


def concat_image_and_label_embedding(images, labels, num_classes, embedding_dim=100):
    h, w = images.shape[-2], images.shape[-1]
    embedding_dim = h * w
    embedding = nn.Embedding(num_classes, embedding_dim)
    labels_embedding = embedding(labels)            # (N, h * w)
    labels_embedding = torch.unflatten(labels_embedding, dim=1, unflattened_size=(1, h, w))
    return torch.cat([images, label_map], dim=1)    # (N, C+1, H, W)
