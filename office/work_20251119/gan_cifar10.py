...
class GAN(nn.Module):
    ...
    def d_loss_fn(self, real_logits, fake_logits):
        # real_labels = torch.ones_like(real_logits) * (1 - self.smoothing)   # label smoothing
        # fake_labels = torch.ones_like(fake_logits) * self.smoothing         # label smoothing

        real_labels = torch.ones_like(real_logits) + 0.1 * torch.rand_like(real_logits)     # noises
        fake_labels = torch.zeros_like(fake_logits) - 0.1 * torch.rand_like(fake_logits)    # noises
