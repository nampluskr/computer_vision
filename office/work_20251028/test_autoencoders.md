...
from models import EncoderV1, DecoderV1     # for MNIST
from models import EncoderV2, DecoderV2     # for CIFAR10
from models import EncoderV3, DecoderV3     # for Oxford Pets
...
model = AutoEncoder(encoder=EncoderV1(in_channels=1, latent_dim=128),
                    decoder=DecoderV1(out_channels=1, latent_dim=128))
...
model = AutoEncoder(encoder=EncoderV2(in_channels=3, latent_dim=128),
                    decoder=DecoderV2(out_channels=3, latent_dim=128))
optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
...
model = AutoEncoder(encoder=EncoderV3(in_channels=3, latent_dim=512),
                    decoder=DecoderV3(out_channels=3, latent_dim=512))
optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
