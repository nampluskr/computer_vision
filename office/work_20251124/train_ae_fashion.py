...
if __name__ == "__main__":
    ...
    if 1:
        ...
        train_loader = get_train_loader(dataset=FashionMNIST(root_dir, "train"), batch_size=BATCH_SIZE)
        test_loader = get_test_loader(dataset=FashionMNIST(root_dir, "test"), batch_size=BATCH_SIZE)

        encoder = Encoder32(in_channels=1, latent_dim=LATENT_DIM)
        decoder = Decoder32(out_channels=1, latent_dim=LATENT_DIM)
    ...
    if 1:
        ...
        encoder = VEncoder32(in_channels=1, latent_dim=LATENT_DIM)
        decoder = Decoder32(out_channels=1, latent_dim=LATENT_DIM)
