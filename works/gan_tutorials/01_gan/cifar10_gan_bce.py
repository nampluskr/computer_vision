...
from datasets import CIFAR10, get_train_loader
...
if __name__ == "__main__":
    ...
    SEED = 42
    DATA_DIR = "/mnt/d/datasets/cifar10"
    BATCH_SIZE = 128

    set_seed(SEED)
    transform = T.Compose([T.ToTensor(), T.Normalize(mean=[0.5]*3, std=[0.5]*3)])
    dataset = CIFAR10(root_dir=DATA_DIR, split="train", transform=transform)
    ...
    generator = Generator(img_size=IMG_SIZE, latent_dim=LATENT_DIM, out_channels=OUT_CHANNELS, base=BASE)
    discriminator = Discriminator(img_size=IMG_SIZE, in_channels=IN_CHANNELS, base=BASE)
