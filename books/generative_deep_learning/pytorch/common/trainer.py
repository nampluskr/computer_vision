...
def predict(model, dataloader):
    ...
    with tqdm(dataloader, desc="Predict", file=sys.stdout, leave=False, ascii=True) as progress_bar:
        for batch in progress_bar:
            batch_size = batch["image"].shape[0]
            total += batch_size
...
class Trainer:
    ...
    def fit(self, train_loader, num_epochs, valid_loader=None):
        return fit(self.model, train_loader, num_epochs, valid_loader=valid_loader)

    def evaluate(self, dataloader):
        return evaluate(self.model, dataloader)

    def predict(self, dataloader):
        return predict(self.model, dataloader)