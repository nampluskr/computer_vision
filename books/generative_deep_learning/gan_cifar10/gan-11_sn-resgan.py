""" SN-ResGAN (full SNGAN): SpectralNorm everywhere + HingeGAN Trainer
- Generator / Discriminator 모두 SpectralNorm 적용
- PixelNorm 제거 (SNGAN은 PixelNorm 사용 안 함)
- ResBlockUp / ResBlockDown 구조는 그대로
- Hinge Loss 사용
- CIFAR10 최적 구조(4→8→16→32)
"""
...
class ResGenerator(nn.Module):
    def __init__(self, latent_dim=128, out_channels=3, base=64):
        ...
        self.fc = spectral_norm(nn.ConvTranspose2d(latent_dim, base * 4, 4, 1, 0))
        ...
        self.out_conv = spectral_norm(nn.Conv2d(base // 2, out_channels, 3, 1, 1))
...
class ResDiscriminator(nn.Module):
    def __init__(self, in_channels=3, base=64):
        ...
        self.linear = spectral_norm(nn.Linear(base * 2, 1))
